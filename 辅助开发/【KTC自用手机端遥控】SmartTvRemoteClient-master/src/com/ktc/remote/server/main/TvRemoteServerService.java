...
import com.ktc.remote.server.update.ClientUpgrade;
import com.ktc.remote.server.update.DownloadThread;
import com.ktc.remote.server.utils.MyUtils;
import com.ktc.remote.server.utils.PackageUtils;
import com.ktc.tvremote.client.R;

public class TvRemoteServerService extends Service {
	private String str_localip;
	private String str_sendip;
	private boolean isGet;

	private ImageView img_mouseCursor = null;
	...
	public void onCreate() {
		...
		startForeground(1, new Notification());// ��߷������ȼ� ��ֹ�����ڴ�ʱ��ɱ��
		...
		str_localip = MainData.getLocalIpAddress();// ��ȡ����IP
		Log.v(MainData.TAG, "localip:" + str_localip);

		str_sendip = "255.255.255.255";

		Log.v(MainData.TAG, "sendip:" + str_sendip);
		...
		String content = String.format("ip=%s;name=%s;id=%s", str_localip,
				serverName, MainData.getDeviceId(this));
		MainData.sendMessage(str_sendip, MainData.CMD_SERVER_INIT, content);// �㲥��Ϣ
		...
		mDownloadThread = new DownloadThread(this.getApplicationContext());
	...
	public void onDestroy() {
		...
		mDownloadThread.bExitFlag = true;
			...
			while (isGet) {
				...
				case MainData.CMD_CLIENT_KEY: {
					...
					if (length == 4) {
						...
						Log.v(MainData.TAG,
								"CMD_CLIENT_KEY:" + String.valueOf(keyCode));
						MainData.sendKeyCodeToSystem(keyCode);
					}
				}
					break;
				case MainData.CMD_CLIENT_SETNAME:// ��������
				{
					int length = MainData.byteArrayToInt(data, 4);
					if (length > 0) {
						...
						String content = String.format("ip=%s;name=%s;id=%s",
								str_localip, name, MainData.getDeviceId(this));
				...
				// Log.v(MainData.TAG, "SEND_KEY_EVENT:" +
	...
	private static byte[] bitmapToBytes(Bitmap bitmap) {
		...
		bitmap.compress(Bitmap.CompressFormat.PNG, 100, os);// ����PNG���кܶೣ����ʽ����jpeg�ȡ�
	...
	private void doMouseClick(String offsetString) {
		try {
			initMouseView();
			WindowManager.LayoutParams wlp = (WindowManager.LayoutParams) img_mouseCursor
					.getLayoutParams();
			int x = wlp.x;
			int y = wlp.y;
	...
	private void doMouseMove(String offsetString) {
		...
		if (ss.length == 2) {
			...
			wm.updateViewLayout(img_mouseCursor, wlp);
	...
	private boolean uninstallApp(String packageName) {
		try {
			...
			if (packageName != null && packageName.length() > 0) {
				...
				String appName = appInfo.loadLabel(this.getPackageManager())
						.toString();
				PackageUtils.silentUninstall(this.getApplicationContext(),
						packageName, appName,
						new PackageUtils.InstallApkCallback() {

							@Override
							public void onUninstallFinished(String packageName,
									String appName, int resultCode) {
								// TODO Auto-generated method stub

								String fmt_ok = getApplicationContext()
										.getResources().getString(
												R.string.format_uninstall_ok);
								String fmt_error = getApplicationContext()
										.getResources()
										.getString(
												R.string.format_uninstall_failed);
								String s = "";
								if (resultCode == 1)
				...
				PackageUtils.silentUninstall(this.getApplicationContext(),
						...
						new PackageUtils.InstallApkCallback() {
							...
							public void onUninstallFinished(String packageName,
								...
								String s = "";
								if (resultCode == 1)
									s = String.format("%s ж�سɹ���", appName);
								else
									s = String.format("%s ж��ʧ�ܣ�", appName);
		...
		try {
			...
			for (int i = 0; i < packages.size(); i++) {
				...
				if (add) {
					...
					Drawable appIcon = pi.applicationInfo
							.loadIcon(getPackageManager());
					BitmapDrawable bd = (BitmapDrawable) appIcon;
					Bitmap bmp = bd.getBitmap();
					byte[] bmp_data = bitmapToBytes(bmp);

					String bmp_string = Base64.encodeToString(bmp_data,
							Base64.DEFAULT);
	...
	private void initMouseView() {
		if (img_mouseCursor != null) {
			img_mouseCursor.setVisibility(View.VISIBLE);
			return;
		}
		...
		WindowManager.LayoutParams wlp = new WindowManager.LayoutParams(
				...
				// WindowManager.LayoutParams.WRAP_CONTENT,
				w, h, WindowManager.LayoutParams.TYPE_SYSTEM_ALERT,
	...
	Handler _handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			if (msg.what == MSG_SHOW_TIPS) {
				...
				Toast.makeText(
						TvRemoteServerService.this.getApplicationContext(),
				...
				if (img_mouseCursor != null)
					img_mouseCursor.setVisibility(View.INVISIBLE);
			}
		}
	};
