...
public class CopyOfCircleProgress extends View {
	...
	private DecimalFormat fnum = new DecimalFormat("#.0");// 格式为保留小数点后一�?

	public CopyOfCircleProgress(Context context) {
		super(context);
		init(null, 0);
	}

	public CopyOfCircleProgress(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(attrs, 0);
	}

	public CopyOfCircleProgress(Context context, AttributeSet attrs, int defStyle) {
	...
	private void init(AttributeSet attrs, int defStyle) {
		...
		mColorWheelPaint.setAntiAlias(true);// 去锯�?
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		int min = Math.min(width, height);// 获取View�?短边的长�?
		setMeasuredDimension(min, min);// 强制改View为以�?短边为长度的正方�?
		circleStrokeWidth = Textscale(40, min);// 圆弧的宽�?
	...
	public class BarAnimation extends Animation {
		...
		/**
		 * 每次系统调用这个方法时， 改变mSweepAnglePer，mPercent，stepnumbernow的�?�，
		...
		protected void applyTransformation(float interpolatedTime,
			...
			if (interpolatedTime < 1.0f) {
				mPercent = Float.parseFloat(fnum.format(interpolatedTime
						* stepnumber * 100f / stepnumbermax));// 将浮点�?�四舍五入保留一位小�?
